...
|---|---|---|---|---|
| 2026-08-21 | локальный, из seed-данных | counts совпали, схема цела | tequilex | Первый прогон на текущей C2C-схеме. Дамп снят теми же ключами `pg_dump`, что и в `scripts/backup.sh`, развёрнут в `restore_test`, проверочные запросы из §2 отработали: 12 таблиц, 4 записи в журнале миграций, счётчики сошлись один в один. **S3-часть не проверялась** — см. ниже. |
...
`NEEDS REVIEW`: проверена только та половина процедуры, что работает с
Postgres, — снятие дампа, восстановление и сверка. **Передача через S3 ни разу
не прогонялась**: раздел 1 (`aws s3 ls` и `aws s3 cp`), загрузка из
`scripts/backup.sh` и суточный запуск контейнера `backup` требуют заполненных
`BACKUP_S3_*` и живого бакета. Пока это не проверено, нельзя утверждать, что в
бакете лежит рабочий дамп, — только что процедура восстановления корректна.
