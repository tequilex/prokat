...
`TZ` задаёт зону контейнера `app` — в ней форматируются все даты интерфейса:
заявки, календарь, время сообщений. Без неё контейнер живёт в UTC, и время
показывается на три часа раньше московского. Хостовая настройка сюда не
наследуется: `toLocaleString` смотрит на зону процесса.
